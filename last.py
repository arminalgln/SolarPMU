...
features = ['Ghi','CloudOpacity']
output_criteria = ['RealPower']

selected_data = data[['Ghi','CloudOpacity','RealPower']]
...
newpmu.opt_ls_mtr(optimizer='adam',
                                loss='mse',
                                metric='mse')
newpmu.model.load_weights('models/w500_ghi_cloud')

#%%
pred = pmu_forecaster.solar_predict(x_test)
#%%
for i, k in enumerate(pred):
    print(i)
    plt.plot(x_test[i][:,0], color = 'magenta')
    plt.plot(x_test[i][:,1], color = 'cyan')
    # plt.plot(x_train[i][:,2],  color = 'orange')
    plt.plot(y_test[i], color = 'blue')
    plt.plot(pred[i], color = 'red')
    plt.legend(['Ghi','cloud','real','pred'])
    plt.savefig('figs/pred/test_cloud_ghi/' + str(i) + '.png')
    plt.show()
...
for i,j in enumerate(x_train):
    ...
    train_mse.append(np.sqrt(pmu_forecaster.solar_eval(x_train[i:i+1], y_train[i:i+1])[0]))

test_mse = list()
for i,j in enumerate(x_test):

    test_mse.append(np.sqrt(pmu_forecaster.solar_eval(x_test[i:i+1], y_test[i:i+1])[0]))
#%%
...
print(count/main_pred.shape[0])
    ...
    # plt.plot(main_pred[i], color = 'blue')
    # plt.plot(main_act[i], color = 'red')
    # plt.show()
